...
module.exports = function(user) {
  ...
  return Promise.resolve(
    request
    .get('https://www.googleapis.com/drive/v3/files?pageSize=5&orderby=modifiedTime&q=mimeType+%3D+"application%2Fvnd.google-apps.document"')
    ...
    .then(files => {
      let promises = files.map(file => {
        return request
        .get(`https://www.googleapis.com/drive/v3/files/${file.googleID}/comments?fields=comments&pageSize=100&quotaUser=${Math.random()}`)
        .set('Authorization', `Bearer ${user.accessToken}`)
      })
      return Promise.all(promises)
    })
